...
function toUserinfoManagement(req, res, next){
    ...
    db.any("SELECT * FROM userinfo")
        .then((result)=> {
            if(result[0] === undefined){
                promptinfo="没有用户信息";
                res.render('user.ejs', {title: '用户管理', test: res.locals.islogin, promptinfo: promptinfo, datas: result});
            }else{
...
function addUserinfoGET(req, res, next) {
    keepLogin(req, res);
    res.render('users_add.ejs', {title: 'Add User', test: res.locals.islogin, promptinfo: "添加用户"});
}
                            ...
                            db.any("SELECT * FROM userinfo WHERE username = $1", req.body.username)
                                .then((result)=> {
                                    if(result[0]){
                                        ...
                                        res.render('users_add.ejs',{title:'Add User',test: res.locals.islogin, promptinfo: '添加失败，请重新登录'});
                                })
                        })
                }else
                    res.render('users_add.ejs',{title:'Add User',test: res.locals.islogin, promptinfo: '两次输入密码不一致，请重新设置'});
            } else
                res.render('users_add.ejs',{title:'Add User',test: res.locals.islogin, promptinfo: '该用户名已经存在'});
        })
        .catch((error)=> {
            console.log(error);
        });
    ...
    db.any("SELECT * FROM userinfo WHERE id = $1", req.params.id)
        .then((result)=> {
            if(result[0] === undefined){
                ...
                promptinfo = "修改用户信息";
                res.render('users_update.ejs', {title: 'Update User', test: res.locals.islogin, promptinfo: promptinfo, datas: result});
            }
        })
        .catch((err)=>{
            return next(err);
